...
void parse_and_execute(UI *pui)
{
    ...
    if (icmd == PCLIST) {
        ...
        if ((islot != MX_SLOT) && (Slots[islot].help != 0)) {
    ...
    if (icmd == PCLOAD) {
        ...
        if (cslot == 0) {
            // pcloadso without argument -- this is an error
    ...
    if (isdigit(cslot[0])) {
        ...
        if ((err != 1) || (islot < 0) || (islot >= MX_SLOT)) {
...
void open_ui_conn(int srvfd, int cb_data)
{
    ...
    add_fd(newuifd, PC_READ, receive_ui, (void *) 0);
...
void open_ui_port()
{
    ...
    add_fd(srvfd, PC_READ, open_ui_conn, (void *) 0);
    ...
    char *so_name)   // Shared object file name to add
{
    ...
    pclog(M_NOSLOT, so_name);
    return(-1);
}


/***************************************************************************
 *  add_so_slot()   - Put .so file into Slot specified.  Ignore request
 *  on error or if no empty slots.  The input string is of the form:
 *  <slot>:<filename.so>.  This routine separates this string into the
 *  slot number and the file name.
 ***************************************************************************/
void add_so_slot(
    char *slot_so)   // Slot # and shared object file name to add
{
    char   sofile[MX_SONAME];       // file name
    int    slot;                    // target slot
    int    ret;                     // scanf return value

    ret = sscanf(slot_so, "%d:%s", &slot, sofile);
    if ((ret != 2) || (slot < 0) || (slot >= MX_SLOT) ||
        (strnlen(sofile, MX_SONAME) == MX_SONAME)) {
        // invalid value or format
        pclog(M_BADSLOT, slot_so);
        return;
    }

    strncpy(Slots[slot].soname, sofile, MX_SONAME);
    ...
    SLOT          *pslot)
{
    ...
    *(void **) (&Initialize) = dlsym(handle, "Initialize");
    ...
    if (errmsg != NULL) {
        pclog(M_BADSYMB, "'Initialize'", pslot->soname);
        pslot->soname[0] = (char) 0;  // void this bogus plug-in entry
        return;
    }

    if (Initialize(pslot) < 0) {
        pclog(M_BADDRIVER, pslot->soname);
        pslot->soname[0] = (char) 0;  // void this bogus plug-in entry
        return;
    }
}
