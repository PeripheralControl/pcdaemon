 ...
 *  - Data structures
 **************************************************************/
    // All state info for an instance of an tang25k
typedef struct
{
    ...
    SLOT *pslot)       // points to the SLOT for this peripheral
{
    ...
    if (pctx == (TNG4KDEV *) 0) {
        ...
        pclog("memory allocation failure in tang25k initialization");
        return (-1);
    }
...
{
    TNG4KDEV *pctx;    // our local info
    PC_PKT    pkt;     // send write and read cmds to the tang25k
...
/**************************************************************
 * getdriverlist():  - Read the list of peripheral IDs in the tang25k
 **************************************************************/
