 ...
 *  Name: line7.c
 *
 *  Description: Driver for HandAssemble visilbe light 7 channel line sensor
 *
 *  Hardware Registers:
 *    0-6 :    - time for 0->1 or 1->0 transition
 *    7   :    - LED brightness control as 8 bit PWM
 *    8   :    - Config
 ...
 * 
 *  Resources:
 *    lineval  - times as 7 two digit hex numbers
 *    ledpwm   - LED brightness as a number in the range of 0 to 255
 *    config   - polarity, clock rate, update period
 */

/*
 * Copyright:   Copyright (C) 2014-2026 Demand Peripherals, Inc.
 *              All rights reserved.
 *
...
/*
 *    The line7 provides 7 channels of input from a Resistor/Capacitor
 ...
 *       The line7 board replaces one sensor in the RCC driver with an output
 *    to control the brightness of the 8 LEDs next to the sensors.
 *      
 */
 ...
 *  - Limits and defines
 **************************************************************/
        // LN7 register definitions
#define LN7_DATA            0x00
#define LN7_LEDPWM          0x07
#define LN7_CONFIG          0x08
#define NPINS               7
        // resource names and numbers
#define FN_DATA             "ln7val"
#define FN_LEDPWM           "ledpwm"
 ...
 *  - Data structures
 **************************************************************/
    // All state info for an instance of an line7 sensor
typedef struct
{
    ...
    uint8_t  ledpwm;   // LED brightness in range of 0 to 255
...
static void noAck(void *, LN7DEV *);
static void sendconfigtofpga(LN7DEV *, int *plen, char *buf);
...
{
    LN7DEV  *pctx;    // our local device context

    // Allocate memory for this peripheral
    pctx = (LN7DEV *) malloc(sizeof(LN7DEV));
    if (pctx == (LN7DEV *) 0) {
        // Malloc failure this early?
        pclog("memory allocation failure in ln7 initialization");
        return (-1);
    }

    // Init our LN7DEV structure
    ...
    pctx->ledpwm = 0;          // LEDs are off at start
    ...
    pslot->rsc[RSC_LEDPWM].name = FN_LEDPWM;
    pslot->rsc[RSC_LEDPWM].flags = IS_READABLE | IS_WRITABLE;
    pslot->rsc[RSC_LEDPWM].bkey = 0;
    pslot->rsc[RSC_LEDPWM].pgscb = userconfig;
    pslot->rsc[RSC_LEDPWM].uilock = -1;
    pslot->rsc[RSC_LEDPWM].slot = pslot;
    ...
    int      len)      // number of bytes in the received packet
{
    ...
    pctx = (LN7DEV *)(pslot->priv);  // Our "private" data
    ...
    // the config or LED brightness.
    // Packet has one byte per sensor input pin.
    if ((pkt->reg != LN7_DATA) || (pkt->count != NPINS)) {
        pclog("invalid ln7 packet from board to host");
        return;
    }
    ...
    if (prsc->bkey != 0) {
        qlen = sprintf(qstr, "%02x %02x %02x %02x %02x %02x %02x\n", pkt->data[0],
    ...
    char    *buf)
{
    ...
    int      brightness; // new value of the LED brightness

    pctx = (LN7DEV *) pslot->priv;

    if ((cmd == PCGET) && (rscid == RSC_LEDPWM)) {
        // Give LED PWM value to user
        ret = snprintf(buf, *plen, "%d\n", pctx->ledpwm);
        *plen = ret;  // (errors are handled in calling routine)
        return;
    }
    else if ((cmd == PCSET) && (rscid == RSC_LEDPWM)) {
        // Brightness is in range of 0 to 255
        ret = sscanf(val, "%d", &brightness);
        if ((ret != 1) || (brightness < 0) || (brightness > 255)) {
            ret = snprintf(buf, *plen, E_BDVAL, pslot->rsc[rscid].name);
            *plen = ret;
            return;
        }
        pctx->ledpwm = brightness;
    }
    else if ((cmd == PCGET) && (rscid == RSC_CONFIG)) {
        // Give config to user
...
static void sendconfigtofpga(
    LN7DEV   *pctx,    // This peripheral's context
    int      *plen,    // size of buf on input, #char in buf on output
    char     *buf)     // where to store user visible error messages
{
    PC_PKT   pkt;      // send write and read cmds to the ln7
    ...
    pkt.reg = LN7_LEDPWM;      // send LED brightness and config
    pkt.count = 2;
    pkt.data[0] = pctx->ledpwm;
    pkt.data[1] = (pctx->polarity << 6) + (pctx->clksrc << 4) + pctx->update;
