    ...
    SLOT *pslot)       // points to the SLOT for this peripheral
{
    ...
    if (pctx == (TIFDEV *) 0) {
        ...
        pclog("memory allocation failure in tif initialization");
        return (-1);
    }
    ...
    int     len)       // number of bytes in the received packet
{
    ...
    if ((pkt->cmd & PC_CMD_OP_MASK) == PC_CMD_OP_READ) {
        ...
        if ((pkt->reg != TIF_R_KEYPAD) || (pkt->count != 2)) {
            pclog("invalid tif read response from board to host");
            return;
        }
    ...
    if (pkt->reg == TIF_R_FIFO) {
        ...
        if (rcount > pctx->idx) {  // more than sent?????
            pclog("invalid tif fifo write response from board to host");
            return;
        }
